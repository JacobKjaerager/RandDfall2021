...
class TABL(Layer):
    ...
    def call(self, x):
        ...
        # MS change, from K.eye to np.identity - theoretically the same
        W = self.W-self.W*np.identity(self.in_shape[2],dtype='float32')+np.identity(self.in_shape[2],dtype='float32')/self.in_shape[2]
        # calculate attention 
